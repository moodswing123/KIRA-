...
async function getChatContext(sock, message) {
  ...
  try {
    ...
    const senderParticipant = participants.find(p => sameJid(p.id, sender));
    const botParticipant = participants.find(p => sameJid(p.id, context.botJid));
    context.isSenderAdmin = senderParticipant?.admin === 'admin' || senderParticipant?.admin === 'superadmin';
    context.isBotAdmin = botParticipant?.admin === 'admin' || botParticipant?.admin === 'superadmin';
  } catch (_) {
...
async function isGroupAdmin(sock, jid, sender) {
  try {
    ...
    return meta.participants.some(
      p => sameJid(p.id, sender) && (p.admin === 'admin' || p.admin === 'superadmin')
    );
  } catch { return false; }
}
...
module.exports = {
  ...
  isGroupAdmin, resolveIsOwner, toggleEmoji,
